...
import { fetchStalls, checkStallName } from "../services/manageStalls.service";
...
export default function ManageStalls() {
  ...
  const checkGridPosition = (row, col, excludeId = null) => {
    return stalls.some((stall) => {
      ...
      if (excludeId && stallId === excludeId) {
        return false;
      }
  ...
  const handleChange = (e) => {
    ...
    if (name === "name") {
      ...
      if (parsed) {
        ...
        setGridError(
          "Stall name must be in format: Letter + Number (e.g., A4, B12). Number must be 1 or higher.",
        );
      } else {
        setGridError("");
        updates.gridRow = "";
    ...
    if (name === "gridRow" || name === "gridCol") {
      const newRow =
        name === "gridRow" ? parseInt(value) || "" : formData.gridRow;

      const newCol =
        name === "gridCol" ? parseInt(value) || "" : formData.gridCol;

      if (newRow && newCol && checkGridPosition(newRow, newCol, editingId)) {
        setGridError(
          `Position (Row: ${newRow}, Col: ${newCol}) is already occupied`,
        );
      } else {
        setGridError("");
      }
  ...
  const handleAdd = async (e) => {
    ...
    if (nameTaken) {
      setErrorMessage(
        "Stall name already exists. Please choose a different name.",
      );
      return;
    }
    ...
    if (!formData.gridRow || !formData.gridCol) {
      setErrorMessage(
        "Please enter a valid stall name (e.g., A4, B12) to set grid position.",
      );
      return;
    }
    ...
    try {
      await httpRequest({
        url: "https://localhost:8443/api/stalls",
        method: "POST",
        headers: {
      ...
      console.error("Add stall error:", err);

      const msg =
        err?.response?.data?.message || err?.message || "Failed to add stall.";
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await httpRequest({
        url: `https://localhost:8443/api/stalls/${editingId}`,
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleRemoveConfirm = async () => {
    ...
    try {
      await httpRequest({
        url: `https://localhost:8443/api/stalls/${removeConfirmId}`,
        method: "DELETE",
      });

      showSuccess("Stall removed successfully");

      if (editingId === removeConfirmId) {
        resetForm();
      }
  ...
  const handleUnreserveConfirm = async () => {
    ...
    try {
      await httpRequest({
        url: `https://localhost:8443/api/stalls/${unreserveConfirmId}/unreserve`,
        method: "PATCH",
      });
      ...
      console.error("Unreserve stall error:", err);

      const msg =
        err?.response?.data?.message ||
        err?.message ||
        "Failed to unreserve stall.";
      ...
      {removeConfirmId != null && (
        <div className="modal-overlay" onClick={handleRemoveCancel}>
          <div className="modal-confirm" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              Are you sure you want to remove this stall? If it is reserved, the
              vendor will be notified by email.
            </p>
      ...
      {unreserveConfirmId != null && (
        <div className="modal-overlay" onClick={handleUnreserveCancel}>
          <div className="modal-confirm" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              Are you sure you want to change this stall from Reserved to
              Available? The vendor will be notified by email and their booking
            ...
            <div className="form-group">
              ...
              {hasNameError && (
                <span className="field-error">
                  This stall name is already in use.
                </span>
              )}

              {gridError && <span className="field-error">{gridError}</span>}

              <span className="field-hint">
                Format: Letter + Number (e.g., A4, B12)
              </span>
            </div>
