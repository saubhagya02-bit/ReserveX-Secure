...
let sharedDashboardPromise = null;

const HomePage = () => {
  const navigate = useNavigate();

  const { user, refreshUser, httpRequest } = useContext(AuthContext);
  ...
  useEffect(() => {
    ...
    if (!sharedDashboardPromise) {
      sharedDashboardPromise = (async () => {
        await refreshUser();
        return await getMyReservations(httpRequest);
      })().finally(() => {
        setTimeout(() => {
  ...
  const loadDashboard = async () => {
    try {
      await refreshUser();
      const data = await getMyReservations(httpRequest);
  ...
  const handleSaveGenres = async (reservationId, payloadArray) => {
    setSaving(true);
    try {
      const response = await updateReservationGenres(payloadArray, httpRequest);
          ...
          res.id === reservationId
            ? {
                ...
                stalls: res.stalls.map((s) => ({
                  ...s,
                  genres:
                    payloadArray.find((p) => p.stallId === s.id)?.genres || [],
                })),
              }
            : res,
        ),
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 border-b border-slate-700/50 pb-8">
            <div>
              ...
              <p className="text-slate-400 text-sm md:text-base max-w-2xl">
                ...
                . The largest exhibition in Sri Lanka, connecting millions of
                readers with publishers like you.
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
            ...
            <div className="w-full bg-slate-100 rounded-full h-2.5 mt-2 overflow-hidden border border-slate-200/50">
              <div
                ...
                style={{
                  width: `${Math.min(((user?.noOfCurrentBookings ?? 0) / 3) * 100, 100)}%`,
                }}
              ></div>
            </div>
          ...
          <StatCard
            ...
            value={
              reservations.filter(
                (res) => res.status?.toUpperCase() === "APPROVED",
              ).length
            }
            ...
            {reservations.length === 0 && !loadingData ? (
              <div className="text-center py-12 bg-white rounded-xl border border-dashed border-slate-300">
                ...
                <p className="text-slate-500 max-w-sm mx-auto mb-6">
                  You haven't booked any stalls for the upcoming book fair.
                ...
                <table className="w-full text-left text-sm text-slate-600">
                  <thead className="bg-slate-50/80 text-slate-500 uppercase tracking-wider text-xs border-b border-slate-200">
                    <tr>
                      ...
                      <th className="px-8 py-4 font-semibold text-slate-600 text-right">
                        Actions
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {reservations.flatMap((res) =>
                      res.stalls?.map((stall) => (
                        ...
                        >
                          <td className="px-8 py-6 font-mono text-slate-900 font-bold text-base">
                            #{res.id}
                          </td>
                          ...
                          <td className="px-8 py-6">
                            <span
                              className={`px-2 py-1 rounded-full font-bold text-[11px] uppercase tracking-wider border ${
                                res.status?.toUpperCase() === "APPROVED"
                                ...
                                return allGenres.length > 0 ? (
                                  <>
                                    {allGenres.slice(0, 3).map((g) => (
                                      <span
                                        key={g}
                                        className="px-2 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded border border-blue-100 shadow-sm"
                                      >
                                        {g}
                                      </span>
                                    ))}
...
const StatCard = ({ title, value, icon, bg }) => (
  <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex items-start justify-between hover:shadow-md transition-shadow">
    ...
    <div className={`p-3 rounded-lg ${bg} bg-opacity-50`}>{icon}</div>
  </div>
);
