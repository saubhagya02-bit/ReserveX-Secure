...
const StallMap = () => {
  const { user, httpRequest } = useContext(AuthContext);
  ...
  const handleConfirmReservation = async () => {
    setIsReserving(true);
    try {
      const response = await createReservation(selectedStalls, httpRequest);
    ...
    if (isSelected) {
      ...
      if (selectedStalls.length >= REMAINING_QUOTA) {
        if (REMAINING_QUOTA <= 0) {
          ...
          toast.error(
            `You have ${existingBookings} active reservations. You can only select ${REMAINING_QUOTA} more stalls.`,
          );
        }
        return;
      }
      ...
      <div className="w-full max-w-5xl mx-auto mb-4 bg-white rounded-xl shadow-sm border border-slate-200 p-4 md:p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2 md:gap-3 w-full md:w-auto">
          ...
          <div className="flex-1 md:flex-none flex flex-col items-center px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-lg shadow-sm">
            ...
            <span className="font-bold text-emerald-700 leading-none mt-1">
              {stats.available}
            </span>
          </div>
          ...
          <div className="flex-1 md:flex-none flex flex-col items-center px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg">
            ...
            <span className="font-bold text-slate-500 leading-none mt-1">
              {stats.reserved}
            </span>
          </div>
        </div>
      ...
      <div className="lg:flex lg:flex-col lg:justify-center items-center h-full">
        <div className="overflow-auto mb-6 p-5 max-w-5xl w-full max-h-[70vh] h-full border border-slate-200 rounded-lg bg-white">
          ...
          <div className="flex flex-wrap gap-2 mb-4 p-2 bg-slate-50 border border-slate-100 rounded-lg">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center px-2">
              Filter
            </span>
            <button
              onClick={() => setActiveFilter("ALL")}
              className={`px-3 py-1.5 text-xs font-bold rounded-md transition ${activeFilter === "ALL" ? "bg-slate-800 text-white" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-100"}`}
            >
              All
            </button>
            <button
              onClick={() => setActiveFilter("AVAILABLE")}
              className={`px-3 py-1.5 text-xs font-bold rounded-md transition ${activeFilter === "AVAILABLE" ? "bg-emerald-600 text-white shadow-sm shadow-emerald-200" : "bg-white text-emerald-700 border border-emerald-200 hover:bg-emerald-50"}`}
            >
              Available
            </button>
            <button
              onClick={() => setActiveFilter("SMALL")}
              className={`px-3 py-1.5 text-xs font-bold rounded-md transition ${activeFilter === "SMALL" ? "bg-blue-600 text-white shadow-sm shadow-blue-200" : "bg-white text-blue-700 border border-blue-200 hover:bg-blue-50"}`}
            >
              Small
            </button>
            <button
              onClick={() => setActiveFilter("MEDIUM")}
              className={`px-3 py-1.5 text-xs font-bold rounded-md transition ${activeFilter === "MEDIUM" ? "bg-purple-600 text-white shadow-sm shadow-purple-200" : "bg-white text-purple-700 border border-purple-200 hover:bg-purple-50"}`}
            >
              Medium
            </button>
            <button
              onClick={() => setActiveFilter("LARGE")}
              className={`px-3 py-1.5 text-xs font-bold rounded-md transition ${activeFilter === "LARGE" ? "bg-orange-600 text-white shadow-sm shadow-orange-200" : "bg-white text-orange-700 border border-orange-200 hover:bg-orange-50"}`}
            >
              Large
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
          <div className="flex flex-col items-center text-center relative group">
            ...
            <p className="text-sm text-slate-500 leading-relaxed px-4">
              ...
              on the map. You can select up to a maximum of 3 stalls for your
              business.
            </p>
          ...
          <div className="flex flex-col items-center text-center relative group">
            ...
            <p className="text-sm text-slate-500 leading-relaxed px-4">
              Review your total cost and click reserve. You will receive an
              official QR code receipt via email.
            </p>
            {/* Desktop connecting line */}
            <div className="hidden md:block absolute top-7 left-[60%] w-[80%] h-[2px] border-t-2 border-dashed border-slate-200"></div>
          </div>
          ...
          <div className="flex flex-col items-center text-center group">
            ...
            <p className="text-sm text-slate-500 leading-relaxed px-4">
              ...
              to assign specific book genres (like Sci-Fi or Romance) to each
              individual stall!
            </p>
          </div>
        </div>
